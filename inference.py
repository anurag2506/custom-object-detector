...
COLORS = {
    ...
    4: (255, 255, 0),    # Speed Limit 30 - Cyan
    5: (255, 0, 255),    # Stop Sign - Magenta
}
...
def detect_image(model, image_path, device, conf_thresh=0.5, output_path=None):
    ...
    boxes, labels, scores = postprocess_detections(outputs, orig_w, orig_h, conf_thresh=conf_thresh)
...
@torch.no_grad()
def create_gif(model, image_folder, device, conf_thresh=0.5, output_path="detection.gif", max_frames=50):
    import glob
...
if __name__ == "__main__":
    ...
    parser.add_argument("--weights", type=str, default="output/best.pth", help="Model weights path")
